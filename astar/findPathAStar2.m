function path = findPathAStar2( att, obs, x_map, y_map, start, goal )
...
if(length(att)~=0)
    for i=1:length(att)
       att{i}(:,1) =  att{i}(:,1) - x_bias;
       att{i}(:,2) =  att{i}(:,2) - y_bias;
    end
end

%%
costmap_att_init = zeros(max(x_map),max(y_map));
if (length(att) == 0)
    costmap_att = costmap_att_init;
    
end

for i=1:length(att)
    costmap_att = occupyAttMap(costmap_att_init,att{i});
    costmap_att_init = costmap_att;
end
...
mapHeuristic = findHeuristicMap2(costmap_att, size(costmap_init),goal); % heuristic map with euclidian distance
searchList = [];
...
[costmap_update, searchListNew] = updateCostMap2(costmap_att, costmap, mapHeuristic, isSearchedMap, node);
...
while(1)
    ...
    [costmap_update, searchListNew] = updateCostMap2( costmap_att, costmap, mapHeuristic, isSearchedMap, node(size(node,1),:));
    costmap = costmap_update;
