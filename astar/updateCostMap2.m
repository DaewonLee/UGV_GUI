...
function [costmap_update, search_list_new] = updateCostMap2(costmap_att, costmap, mapHeuristic, isSearchedMap, node)
           ...
           if ((p(1) + n > 0) && (p(2) + k > 0) && (p(1) + n <= size(costmap,1)) && (p(2) + k  <= size(costmap,2))) % check if the node is inside of the map
               % if (( isSearchedMap(p(1)+n, p(2)+k) == 0) && costmap(p(1)+n, p(2)+k) ~= 1000) % 1000 : obstacle
                if (( isSearchedMap(p(1)+n, p(2)+k) == 0) && costmap(p(1)+n, p(2)+k) ~= 1000) % 1000 : obstacle
                    ...
                    if(  costmap_att(p(1)+n, p(2)+k) == 1)
                        value = att_const * (cost_prev + mapHeuristic(p(1)+n, p(2)+k));
                    else
